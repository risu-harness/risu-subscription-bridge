...
function fixture(run) {
  ...
  const control={alive:true,account:async()=>({connected:true}),models:async()=>[{model:'test-model',isDefault:true,supportedReasoningEfforts:[{reasoningEffort:'low'}]}],shutdown(){}};
  return {adapter:new CodexExec({binary:'codex',env:{CODEX_HOME:'/tmp/example'},cwd:'/empty',control,spawnProcess,writeInstructions:async()=>{}}),child,get captured(){return captured;}};
}
...
test('exec uses stdin/ephemeral; emits completed message once and excludes reasoning',async()=>{
  ...
  let text='';const r=await f.adapter.generate({...req,effort:'low',verbosity:'high'},{signal:new AbortController().signal,delta:s=>text+=s});
  assert.ok(f.captured.args.includes('model_reasoning_effort="low"'));assert.ok(f.captured.args.includes('model_verbosity="high"'));
