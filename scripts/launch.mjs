...
async function reuse() {
  ...
  if(options.adapter && options.adapter !== existing.adapter)throw Error(`실행 중인 모드는 ${existing.adapter || 'unknown'}입니다. --adapter ${options.adapter} --restart로 전환하세요.`);
...
const existing=await findInstance(runtime,ports);
const backend=options.adapter || existing?.adapter || 'exec';
let start=!existing;
let port=requested;
if(existing) {
  console.log(`실행 중인 브리지: http://127.0.0.1:${existing.port}/`);
  const action=options.action || await chooseAction();
...
if(start) {
  ...
  console.log(`브리지 시작 · http://127.0.0.1:${port} · ${backend}`);
  const child=spawn(cfg.node,[join(cfg.source,'src','main.mjs')],{cwd:cfg.source,stdio:'inherit',env:{...process.env,BRIDGE_CODEX_BIN:cfg.codex,BRIDGE_DATA_DIR:runtime,BRIDGE_PORT:String(port),BRIDGE_ADAPTER:backend,BRIDGE_OPEN_BROWSER:process.env.BRIDGE_OPEN_BROWSER??'1'}});
