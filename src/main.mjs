...
await control.init();
const settings = new Settings(runtime); await settings.init();
const adapter = configuredAdapter(control,settings);
...
function stop() {
  if (stopping) return;
  stopping = true;
  server.close(); server.closeAllConnections(); adapter.shutdown(); instanceGuard.close();
}
