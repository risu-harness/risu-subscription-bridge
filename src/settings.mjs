...
export class Settings {
  constructor(runtime,backend='exec'){this.path=join(runtime,'generation-settings.json');this.value={backend,model:'subscription-default',effort:'',verbosity:'',instructions:''};}
  async init(){try{this.value={...this.value,...JSON.parse(await readFile(this.path,'utf8'))};}catch(e){if(e.code!=='ENOENT')throw e;}}
  async save(value,models){
    const allowed=['backend','model','effort','verbosity','instructions'];
    ...
    if(!['exec','app-server'].includes(value.backend)||!['','low','medium','high'].includes(value.verbosity)||value.instructions.length>16000)throw new BridgeError('설정 값이 올바르지 않습니다.',400,'settings_invalid');
...
export function configuredAdapter(control,cli,settings){
 return {
  get alive(){return control.alive;},get name(){return settings.value.backend==='exec'?cli.name:control.name;},get delivery(){return settings.value.backend==='exec'?cli.delivery:control.delivery;},
  account:()=>control.account(),models:()=>control.models(),login:()=>control.login(),shutdown:()=>cli.shutdown(),
  generate(request,handlers){const s={...settings.value};const model=request.model==='subscription-default'?s.model:request.model;
   return (s.backend==='exec'?cli:control).generate({...request,model,effort:s.effort,verbosity:s.verbosity,instructions:s.instructions},handlers);
  }
 };
}
