...
export function createBridge({adapter, token, port = 8787, origins = ['https://risuai.xyz', 'https://risuai.net', 'tauri://localhost', 'http://tauri.localhost'], runtime = '', shutdown, settings}) {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (req.method === 'POST' && path === '/internal/settings' && settings) {
        ...
        busy=true;
        try {return json(200,{settings:await settings.save(await readJSON(req),await adapter.models())});}
        finally {busy=false;}
      }
